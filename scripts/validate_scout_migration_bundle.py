...
meta=data.get('_meta',{})
if meta.get('notificationOwnerAfterCutover')!='Microsoft Scout Teams Bot':errors.append('invalid notification owner')
for a in autos:
 name=a.get('name','<unnamed>')
 if a.get('enabled') is not False:errors.append(f'{name}: must be disabled')
 if a.get('teamsNotify')!='never':errors.append(f'{name}: teamsNotify must be never in shadow')
 if a.get('cutoverTeamsNotify') not in {'always','auto'}:errors.append(f'{name}: invalid cutoverTeamsNotify')
